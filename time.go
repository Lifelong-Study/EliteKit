...
func NewAlarmAtSpecificTimeAndInterval(hour, minute int, interval time.Duration) *time.Timer {
	...
	if int(interval) > now.Minute() {
		...
		target = (1 + (now.Minute() / int(interval))) * int(interval)
